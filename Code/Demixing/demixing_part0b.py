...
def main():
    ...
    params = cartesian([lrs, rhos, mus, nests])
    ...
    train_nns_part = partial(train_nns, params=params, train_data=train_data, valid_data=valid_data)

    pool = mp.Pool(processes=8)
    results = pool.map(train_nns_part, xrange(len(params)))

    file_name = "nn_optim_c" + str(c) + ".pkl"
